...
**代码实现** (`foundation_service/services/role_service.py`):
```python
...
**问题**: 缺少预设角色 code 修改保护
- **要求**: 预设角色的 code 不可修改
- **现状**: 代码注释说"简化处理"，但没有实际检查
- **影响**: 如果更新请求中包含 `code` 字段，可能会被修改
- **建议**: 检查 Schema 中是否包含 `code` 字段，如果包含，添加保护逻辑
...
- **要求**: 如果角色已被用户使用，不允许删除
- **现状**: 没有检查角色是否被用户使用
- **影响**: 可能删除正在使用的角色，导致用户失去角色
- **建议**: 在删除前检查 `user_roles` 表中是否有使用该角色的记录
...
1. **用户创建时缺少组织激活状态检查**
   ...
   - 位置: `create_user` 方法
   - 修复: 添加 `if not organization.is_active: raise OrganizationInactiveError()`
