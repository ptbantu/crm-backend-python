...
class OrganizationDomainRelation(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    organization_id = Column(String(36), nullable=False, index=True, comment="组织ID（跨服务，无外键）")
    domain_id = Column(String(36), ForeignKey("organization_domains.id", ondelete="CASCADE"), nullable=False, index=True, comment="领域ID")
