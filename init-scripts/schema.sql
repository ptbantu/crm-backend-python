...
CREATE TABLE IF NOT EXISTS `contract_entities` (
  ...
  `seal_oss_key` varchar(500) DEFAULT NULL COMMENT '印章图片OSS路径（PNG格式，透明背景）',
  `seal_position_x` int DEFAULT NULL COMMENT '印章X坐标（像素）',
  `seal_position_y` int DEFAULT NULL COMMENT '印章Y坐标（像素）',
  `seal_width` int DEFAULT NULL COMMENT '印章宽度（像素）',
  `seal_height` int DEFAULT NULL COMMENT '印章高度（像素）',
  ...
  KEY `idx_seal_oss_key` (`seal_oss_key`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='客户关联国内工商主体(天眼查集成)';
...
CREATE TABLE IF NOT EXISTS `crm_documents` (
  `id` char(36) NOT NULL,
  `document_no` varchar(50) NOT NULL COMMENT '文档编号（如：DOC-20260218-001）',
  `document_type` enum('contract','invoice','quotation','other') NOT NULL COMMENT '文档类型',
  `opportunity_id` char(36) DEFAULT NULL COMMENT '关联商机ID',
  `contract_id` char(36) DEFAULT NULL COMMENT '关联合同ID',
  `invoice_id` char(36) DEFAULT NULL COMMENT '关联发票ID',
  `template_id` char(36) DEFAULT NULL COMMENT '使用的模板ID',
  `entity_id` char(36) NOT NULL COMMENT '签约主体ID',
  `group_id` char(36) DEFAULT NULL COMMENT '文档组ID（用于关联报价单→合同→发票）',
  `title` varchar(255) NOT NULL COMMENT '文档标题',
  `description` text COMMENT '文档描述',
  `draft_oss_key` varchar(500) DEFAULT NULL COMMENT 'Word草稿文件OSS路径',
  `final_oss_key` varchar(500) DEFAULT NULL COMMENT 'PDF/A最终文件OSS路径',
  `file_size_kb` int DEFAULT NULL COMMENT '文件大小（KB）',
  `file_hash` varchar(64) DEFAULT NULL COMMENT 'SHA-256文件哈希（用于完整性验证）',
  ...
  `submitted_by` char(36) DEFAULT NULL COMMENT '提交人ID',
  `approved_at` datetime DEFAULT NULL COMMENT '审批通过时间',
  `approved_by` char(36) DEFAULT NULL COMMENT '审批人ID',
  `rejected_at` datetime DEFAULT NULL COMMENT '拒绝时间',
  `rejected_by` char(36) DEFAULT NULL COMMENT '拒绝人ID',
  `rejection_reason` text COMMENT '拒绝原因',
  `sealed_at` datetime DEFAULT NULL COMMENT '盖章时间',
  `sealed_by` char(36) DEFAULT NULL COMMENT '盖章人ID',
  `finalized_at` datetime DEFAULT NULL COMMENT '最终化时间（转换为PDF/A）',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `created_by` char(36) DEFAULT NULL COMMENT '创建人ID',
  PRIMARY KEY (`id`),
  ...
  KEY `fk_crm_doc_submitted_by` (`submitted_by`),
  KEY `fk_crm_doc_approved_by` (`approved_by`),
  KEY `fk_crm_doc_rejected_by` (`rejected_by`),
  KEY `fk_crm_doc_sealed_by` (`sealed_by`),
  KEY `fk_crm_doc_created_by` (`created_by`),
...
CREATE TABLE IF NOT EXISTS `crm_opp_pipeline_instances` (
  ...
  KEY `idx_status` (`status`),
  KEY `idx_current_stage` (`current_stage_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商机流水线实例表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商机阶段日志表';
...
CREATE TABLE IF NOT EXISTS `customer_documents` (
  ...
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `last_notified_day` int DEFAULT NULL COMMENT 'æœ€åŽé€šçŸ¥çš„å‰©ä½™å¤©æ•°ï¼ˆ5/3/1ï¼‰',
  PRIMARY KEY (`id`),
  ...
  KEY `ix_customer_documents_last_notified` (`last_notified_day`),
...
CREATE TABLE IF NOT EXISTS `customers` (
  ...
  `tianyancha_data` json DEFAULT NULL COMMENT 'å¤©çœ¼æŸ¥ä¼ä¸šæ•°æ®',
  `tianyancha_synced_at` datetime DEFAULT NULL COMMENT 'å¤©çœ¼æŸ¥åŒæ­¥æ—¶é—´',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_customers_tianyancha_synced_at` (`tianyancha_synced_at`),
  KEY `idx_customers_linked_module` (`linked_module`),
...
) ENGINE=InnoDB AUTO_INCREMENT=12003 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS `doc_contract_ext` (
  `id` char(36) NOT NULL,
  `document_id` char(36) NOT NULL COMMENT '关联文档ID',
  `contract_id` char(36) DEFAULT NULL COMMENT '关联合同ID（可选）',
  ...
  `effective_to` date DEFAULT NULL COMMENT '到期日期',
...
CREATE TABLE IF NOT EXISTS `doc_invoice_ext` (
  `id` char(36) NOT NULL,
  `document_id` char(36) NOT NULL COMMENT '关联文档ID',
  `invoice_id` char(36) DEFAULT NULL COMMENT '关联发票ID（可选）',
  `contract_document_id` char(36) DEFAULT NULL COMMENT '关联合同文档ID（用于追溯）',
  ...
  `amount_before_tax` decimal(18,2) DEFAULT NULL COMMENT '税前金额',
  `currency` varchar(10) NOT NULL DEFAULT 'CNY' COMMENT '币种',
  `tax_rate` decimal(5,4) DEFAULT NULL COMMENT '税率',
  ...
  KEY `idx_document_id` (`document_id`),
  KEY `idx_invoice_id` (`invoice_id`),
  KEY `idx_contract_document_id` (`contract_document_id`),
...
CREATE TABLE IF NOT EXISTS `exe_orders` (
  ...
  KEY `idx_org_created` (`organization_id`,`created_at` DESC)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='交付执行订单主表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='订单执行任务拆解表';
...
CREATE TABLE IF NOT EXISTS `fin_bills` (
  ...
  KEY `idx_created_at` (`created_at` DESC)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='企业应收应付账单管理';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='提成计算规则表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='个人提成结算单(月度/年度)';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='内部经营/开票主体配置';
...
CREATE TABLE IF NOT EXISTS `id_sequences` (
  ...
  `sequence` bigint NOT NULL DEFAULT '0' COMMENT '当前序号',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=31 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='ID序号表';
CREATE TABLE IF NOT EXISTS `industries` (
  `id` char(36) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT (uuid()),
  `code` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'è¡Œä¸šä»£ç ',
  `name_zh` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '行业名称（中文）',
  `name_id` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '行业名称（印尼语）',
  `sort_order` int NOT NULL DEFAULT '0' COMMENT 'æŽ’åºé¡ºåº',
  `is_active` tinyint(1) DEFAULT '1' COMMENT 'æ˜¯å¦æ¿€æ´»',
  `description_zh` text COLLATE utf8mb4_unicode_ci COMMENT 'æè¿°ï¼ˆä¸­æ–‡ï¼‰',
  `description_id` text COLLATE utf8mb4_unicode_ci COMMENT 'æè¿°ï¼ˆå°å°¼è¯­ï¼‰',
...
CREATE TABLE IF NOT EXISTS `opportunity_action_logs` (
  ...
  KEY `fk_action_log_created_by` (`created_by`),
  KEY `fk_action_log_updated_by` (`updated_by`),
  CONSTRAINT `fk_action_log_action_config` FOREIGN KEY (`action_config_id`) REFERENCES `sys_pipeline_action_configs` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_action_log_created_by` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_action_log_operator` FOREIGN KEY (`operator_id`) REFERENCES `users` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_action_log_opportunity` FOREIGN KEY (`opportunity_id`) REFERENCES `opportunities` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_action_log_pipeline_log` FOREIGN KEY (`pipeline_log_id`) REFERENCES `opportunity_pipeline_logs` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_action_log_updated_by` FOREIGN KEY (`updated_by`) REFERENCES `users` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商机流水线动作执行日志表';
...
CREATE TABLE IF NOT EXISTS `opportunity_pipeline_logs` (
  ...
  `notes` text COMMENT '备注',
  `extra_data` json DEFAULT NULL COMMENT 'æ‰©å±•æ•°æ®ï¼ˆJSONæ ¼å¼ï¼‰',
  `created_by` char(36) DEFAULT NULL COMMENT '创建人ID',
  `updated_by` char(36) DEFAULT NULL COMMENT '更新人ID',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  ...
  KEY `fk_opp_pipeline_logs_created_by` (`created_by`),
  KEY `fk_opp_pipeline_logs_updated_by` (`updated_by`),
...
CREATE TABLE IF NOT EXISTS `product_document_rules` (
  ...
  `min_file_count` int NOT NULL DEFAULT '1' COMMENT '最小文件数（必填）',
  `max_file_count` int DEFAULT NULL COMMENT '最大文件数（NULL表示不限制）',
  `support_zip` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否支持ZIP文件',
  `zip_extract_mode` varchar(50) DEFAULT NULL COMMENT 'ZIP解压模式：none(不解压), extract(解压后单独校验), both(同时保留ZIP和解压文件)',
  `file_type` varchar(50) DEFAULT NULL COMMENT '文件类型：text, pdf, zip, image, file（兼容现有document_type）',
  ...
  CONSTRAINT `chk_max_file_count` CHECK (((`max_file_count` is null) or (`max_file_count` >= `min_file_count`))),
  CONSTRAINT `chk_min_file_count` CHECK ((`min_file_count` > 0)),
  CONSTRAINT `chk_zip_extract_mode` CHECK (((`zip_extract_mode` is null) or (`zip_extract_mode` in (_utf8mb4'none',_utf8mb4'extract',_utf8mb4'both'))))
...
CREATE TABLE IF NOT EXISTS `requirement_attachment_details` (
  ...
  `parent_attachment_id` char(36) DEFAULT NULL COMMENT '父附件ID（用于ZIP解压场景，外键 → requirement_attachment_details.id）',
  ...
  `is_zip` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否为ZIP文件',
  `is_extracted` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否已解压',
  `extracted_file_count` int DEFAULT NULL COMMENT '解压后的文件数量',
  `zip_extract_path` varchar(500) DEFAULT NULL COMMENT 'ZIP解压路径（如果解压）',
  `validation_status` varchar(50) NOT NULL DEFAULT 'pending' COMMENT '校验状态：pending(待校验), passed(通过), failed(失败)',
  `validation_message` text COMMENT '校验失败原因',
  `validated_at` datetime DEFAULT NULL COMMENT '校验时间',
  `validated_by` char(36) DEFAULT NULL COMMENT '校验人ID（外键 → users.id）',
  `uploaded_by` char(36) DEFAULT NULL COMMENT '上传人ID（外键 → users.id）',
  `uploaded_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '上传时间',
  `deleted_at` datetime DEFAULT NULL COMMENT '删除时间（软删除）',
  PRIMARY KEY (`id`),
  KEY `idx_submission_record` (`submission_record_id`),
  KEY `idx_parent_attachment` (`parent_attachment_id`),
  KEY `idx_file_type` (`file_type`),
  KEY `idx_validation_status` (`validation_status`),
  KEY `idx_is_zip` (`is_zip`),
  KEY `fk_attachment_uploaded_by` (`uploaded_by`),
  KEY `fk_attachment_validated_by` (`validated_by`),
...
CREATE TABLE IF NOT EXISTS `requirement_submission_records` (
  ...
  `product_id` char(36) NOT NULL COMMENT '产品ID（外键 → products.id）',
  `rule_id` char(36) NOT NULL COMMENT '资料规则ID（外键 → product_document_rules.id）',
  `order_id` char(36) DEFAULT NULL COMMENT '订单ID（外键 → orders.id，用于订单场景）',
  `service_record_id` char(36) DEFAULT NULL COMMENT '服务记录ID（外键 → service_records.id，用于服务记录场景）',
  `opportunity_id` char(36) DEFAULT NULL COMMENT '商机ID（外键 → opportunities.id，用于商机场景）',
  `contract_id` char(36) DEFAULT NULL COMMENT '合同ID（外键 → contracts.id，用于合同场景）',
  ...
  `validation_passed` tinyint(1) NOT NULL DEFAULT '0' COMMENT '格式校验是否通过',
  `reviewed_by` char(36) DEFAULT NULL COMMENT '审核人ID（外键 → users.id）',
  `reviewed_at` datetime DEFAULT NULL COMMENT '审核时间',
  ...
  KEY `idx_product_rule` (`product_id`,`rule_id`),
  KEY `idx_order` (`order_id`),
  KEY `idx_service_record` (`service_record_id`),
  KEY `idx_opportunity` (`opportunity_id`),
...
CREATE TABLE IF NOT EXISTS `st_visa_processed_logs` (
  ...
  `email_id` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '邮件ID（唯一标识）',
  `passport_no` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '护照号',
  `customer_name` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '客户姓名',
  `processed_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '处理时间',
  `status` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'success' COMMENT '处理状态：success, failed, skipped',
...
) ENGINE=InnoDB AUTO_INCREMENT=13 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='签证邮件处理日志表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='核心审批流记录表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='组织架构-部门表';
CREATE TABLE IF NOT EXISTS `sys_doc_templates` (
  `id` char(36) NOT NULL,
  `template_code` varchar(50) NOT NULL COMMENT '模板代码（唯一，如：CONTRACT_BJ_BANTU）',
  `template_name` varchar(255) NOT NULL COMMENT '模板名称',
  `template_type` enum('contract','invoice','quotation','other') NOT NULL COMMENT '模板类型',
  `entity_id` char(36) DEFAULT NULL COMMENT '关联签约主体ID（NULL表示通用模板）',
  `language` varchar(20) NOT NULL DEFAULT 'zh' COMMENT '模板语言（zh/id/en）',
  `file_oss_key` varchar(500) NOT NULL COMMENT 'OSS存储路径（Word模板）',
  `file_name` varchar(255) NOT NULL COMMENT '原始文件名',
  `file_size_kb` int DEFAULT NULL COMMENT '文件大小（KB）',
  `placeholders` json DEFAULT NULL COMMENT '占位符列表（解析自模板，如：["customer_name", "amount"]）',
  `description` text COMMENT '模板描述',
  `is_active` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否启用',
  ...
  KEY `idx_is_active` (`is_active`),
  KEY `idx_created_at` (`created_at`),
  KEY `fk_doc_template_created_by` (`created_by`),
  KEY `fk_doc_template_updated_by` (`updated_by`),
...
CREATE TABLE IF NOT EXISTS `sys_pipeline_action_configs` (
  ...
  CONSTRAINT `fk_action_config_stage` FOREIGN KEY (`stage_id`) REFERENCES `sys_pipeline_stages` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='流水线阶段原子动作配置表';
CREATE TABLE IF NOT EXISTS `sys_pipeline_configs` (
  ...
  KEY `idx_biz_type` (`biz_type`),
  KEY `idx_is_active` (`is_active`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商机流水线配置表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='智能搜索索引表';
