-- ============================================================
-- 权限控制系统表结构（安全版本 - 跳过已存在的表和索引）
-- ============================================================
-- 包含：权限点、角色权限关联、菜单、菜单权限关联
-- 支持中印尼语双语

-- ============================================================
-- 1. 修改 roles 表，添加双语字段（如果不存在）
-- ============================================================
SET @dbname = DATABASE();
SET @tablename = 'roles';
...
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
   WHERE TABLE_SCHEMA = @dbname AND TABLE_NAME = @tablename AND COLUMN_NAME = 'name_zh') > 0,
  'SELECT 1',
...
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
   WHERE TABLE_SCHEMA = @dbname AND TABLE_NAME = @tablename AND COLUMN_NAME = 'name_id') > 0,
  'SELECT 1',
...
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
   WHERE TABLE_SCHEMA = @dbname AND TABLE_NAME = @tablename AND COLUMN_NAME = 'description_zh') > 0,
  'SELECT 1',
...
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
   WHERE TABLE_SCHEMA = @dbname AND TABLE_NAME = @tablename AND COLUMN_NAME = 'description_id') > 0,
  'SELECT 1',
...
UPDATE roles SET 
  ...
  description_zh = CASE code
    ...
    ELSE COALESCE(description_zh, description)
  END,
  description_id = CASE code
    ...
    ELSE COALESCE(description_id, description)
  END
WHERE name_zh IS NULL OR name_zh = '';

-- ============================================================
-- 2. Permissions (权限点表) - 只插入数据，不创建表
-- ============================================================
-- 插入基础权限点数据（使用 INSERT IGNORE 避免重复）
...
-- ============================================================
-- 3. Role Permissions (角色权限关联) - 只插入数据
-- ============================================================
-- 为 ADMIN 角色分配所有权限
...
-- 4. Menus (菜单表) - 只插入数据
-- ============================================================
-- 插入基础菜单数据（使用 INSERT IGNORE 避免重复）
-- 注意：这里使用变量来存储菜单ID，以便后续关联
SET @dashboard_id = UUID();
SET @user_mgmt_id = UUID();
SET @org_mgmt_id = UUID();
SET @role_mgmt_id = UUID();
SET @lead_mgmt_id = UUID();
SET @opp_mgmt_id = UUID();
SET @order_mgmt_id = UUID();
SET @finance_mgmt_id = UUID();
SET @system_mgmt_id = UUID();
...
(@dashboard_id, 'dashboard', '工作台', 'Dashboard', '工作台首页', 'Halaman dashboard', NULL, '/dashboard', 'Dashboard', 'dashboard', 10),
(@user_mgmt_id, 'user-management', '用户管理', 'Manajemen Pengguna', '用户管理模块', 'Modul manajemen pengguna', NULL, '/users', NULL, 'user', 100),
(@org_mgmt_id, 'organization-management', '组织管理', 'Manajemen Organisasi', '组织管理模块', 'Modul manajemen organisasi', NULL, '/organizations', NULL, 'team', 200),
(@role_mgmt_id, 'role-management', '角色管理', 'Manajemen Peran', '角色管理模块', 'Modul manajemen peran', NULL, '/roles', NULL, 'safety', 300),
(@lead_mgmt_id, 'lead-management', '线索管理', 'Manajemen Prospek', '线索管理模块', 'Modul manajemen prospek', NULL, '/leads', NULL, 'contacts', 400),
(@opp_mgmt_id, 'opportunity-management', '商机管理', 'Manajemen Peluang', '商机管理模块', 'Modul manajemen peluang', NULL, '/opportunities', NULL, 'project', 500),
(@order_mgmt_id, 'order-management', '订单管理', 'Manajemen Pesanan', '订单管理模块', 'Modul manajemen pesanan', NULL, '/orders', NULL, 'shopping', 600),
(@finance_mgmt_id, 'finance-management', '财务管理', 'Manajemen Keuangan', '财务管理模块', 'Modul manajemen keuangan', NULL, '/finance', NULL, 'dollar', 700),
(@system_mgmt_id, 'system-management', '系统管理', 'Manajemen Sistem', '系统管理模块', 'Modul manajemen sistem', NULL, '/system', NULL, 'setting', 800);

-- 获取已存在的菜单ID（如果已存在）
SELECT id INTO @dashboard_id FROM menus WHERE code = 'dashboard' LIMIT 1;
SELECT id INTO @user_mgmt_id FROM menus WHERE code = 'user-management' LIMIT 1;
SELECT id INTO @org_mgmt_id FROM menus WHERE code = 'organization-management' LIMIT 1;
SELECT id INTO @role_mgmt_id FROM menus WHERE code = 'role-management' LIMIT 1;
SELECT id INTO @lead_mgmt_id FROM menus WHERE code = 'lead-management' LIMIT 1;
SELECT id INTO @opp_mgmt_id FROM menus WHERE code = 'opportunity-management' LIMIT 1;
SELECT id INTO @order_mgmt_id FROM menus WHERE code = 'order-management' LIMIT 1;
SELECT id INTO @finance_mgmt_id FROM menus WHERE code = 'finance-management' LIMIT 1;
SELECT id INTO @system_mgmt_id FROM menus WHERE code = 'system-management' LIMIT 1;
...
-- ============================================================
-- 5. Menu Permissions (菜单权限关联) - 只插入数据
-- ============================================================
-- 为菜单关联权限（使用 INSERT IGNORE 避免重复）
