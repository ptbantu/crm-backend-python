...
class ProductPriceHistoryRequest(BaseModel):
    ...
    price_type: str = Field(..., description="价格类型：cost, channel, direct, list")
    currency: str = Field(..., description="货币：IDR, CNY, USD, EUR")
    amount: Decimal = Field(..., ge=0, description="价格金额")
...
class ProductPriceHistoryUpdateRequest(BaseModel):
    """更新价格请求"""
    amount: Optional[Decimal] = Field(None, ge=0, description="价格金额")
    exchange_rate: Optional[Decimal] = Field(None, gt=0, description="汇率")
