...
|------|-----|
| **ID类型** | 数据库自增整数 |
| **字段类型** | `INT AUTO_INCREMENT` |
| **起始值** | `12000` |
| **格式** | 纯数字（5位及以上） |
| **示例** | `12000`, `12001`, `12002` |
| **说明** | 
- 使用MySQL的AUTO_INCREMENT功能自动生成
- 从12000开始递增
- 不再使用ID生成器
- 所有引用customer_id的外键字段也改为INT类型
...
# 客户ID由数据库自动生成（AUTO_INCREMENT）
# 创建Customer对象时不需要手动设置ID
# 数据库会自动分配：12000, 12001, 12002...
...
MODULE_PREFIXES = {
    "Organization": "ORG",
    # "Customer": "CUS",  # 已改为数据库自增，不再使用ID生成器
    "Order": "ORD",
    # ...
}
...
### Q5: 客户 ID 为什么使用数据库自增而不是ID生成器？

**A:** 
- 客户ID改为数据库自增整数（从12000开始）
- 原因：
  - 简化ID格式，便于记忆和输入
  - 提高性能（数据库原生支持）
  - 减少代码复杂度
- 注意：所有引用customer_id的外键字段也改为INT类型
...
- **2025-12-28**：客户ID改为数据库自增整数（从12000开始），不再使用ID生成器
