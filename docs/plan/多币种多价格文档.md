...
2. **供应商成本版本控制**: 供应商报价变更不影响历史订单的成本核算
3. **多等级多币种价格体系**: 根据客户等级提供不同价格，支持 CNY 和 IDR 双币种
4. **项目执行进度管理**: 标准执行时长、阶段划分、进度预警
5. **精确利润核算**: 支持供应商成本、浮动成本（报销）的精确归集
...
### 3.3 供应商成本版本控制表 (`supplier_cost_history`)

**设计**: 记录供应商对每个服务的成本报价历史，支持版本控制

**核心字段**:
- `product_id`: 服务ID
- `supplier_id`: 供应商ID（`organizations.id` where `organization_type='vendor'`）
- `version`: 版本号
- `cost_cny`: 人民币成本
- `cost_idr`: 印尼盾成本
...
**设计**: 在订单项中记录供应商选择和成本快照

**新增字段**:
- `selected_supplier_id`: 执行该项的供应商ID
...
**业务规则**:
- 订单创建时，必须选择供应商并记录成本快照
- 成本快照用于利润核算，不受后续供应商报价变更影响
...
### 4.3 创建供应商成本版本表 (`supplier_cost_history`)
...
CREATE TABLE `supplier_cost_history` (
  ...
  `supplier_id` CHAR(36) NOT NULL COMMENT '关联供应商ID (对应 organizations表 type=vendor)',
  `version` INT NOT NULL DEFAULT 1 COMMENT '版本号（同一供应商同一服务的版本号递增）',
  ...
  `notes` TEXT COMMENT '备注说明（如：涨价原因、特殊条件等）',
  `created_by` CHAR(36) DEFAULT NULL COMMENT '创建人ID',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  ...
  -- 确保同一供应商同一服务在同一时间段只有一个有效报价
  CONSTRAINT `chk_cost_nonneg` CHECK (
    (COALESCE(`cost_cny`, 0) >= 0) AND 
    (COALESCE(`cost_idr`, 0) >= 0)
  )
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='供应商成本历史表(版本控制)';
```
...
-- 1. 记录选择了哪个供应商
ADD COLUMN `selected_supplier_id` CHAR(36) DEFAULT NULL COMMENT '执行该项的供应商ID',

-- 2. 核心：记录成本来源的版本ID (关联 supplier_cost_history.id)
ADD COLUMN `supplier_cost_history_id` CHAR(36) DEFAULT NULL COMMENT '关联的成本版本ID',

-- 3. 成本快照 (双重保险，防止历史表被删，直接把金额固化在订单行里)
...
### 5.2 场景二：中台 (KK) 派单与供应商选择

**业务场景**: 订单生成后，KK 需要为订单项选择执行供应商
...
1. **查询可用供应商**: 
   ...
   WHERE sch.product_id = ?
     ...
     AND o.is_active = 1
   ORDER BY sch.cost_cny ASC  -- 按成本排序
   ```
2. **决策**: KK 根据速度、利润分配、执行周期长度等因素选择供应商
3. **更新订单项**:
   ```sql
   UPDATE order_items 
   SET 
...
POST /api/v1/order-items/{order_item_id}/assign-supplier
Request: {
  "supplier_id": "...",
  "supplier_cost_history_id": "..."
}
...
3. **创建新报价**:
   ...
   INSERT INTO supplier_cost_history (
     product_id, supplier_id, version, 
     cost_cny, cost_idr,
     effective_start_at, is_current, created_by
   ) VALUES (
     ?, ?, 
     (SELECT MAX(version) + 1 FROM supplier_cost_history WHERE product_id = ? AND supplier_id = ?),
     ?, ?, NOW(), 1, ?
   )
   ```
4. **影响分析**:
   - **新订单**: 自动关联到新报价（成本更高）
   - **旧订单**: `order_items.supplier_cost_history_id` 仍指向旧报价，利润核算不受影响

**API 设计**:
```
POST /api/v1/suppliers/{supplier_id}/products/{product_id}/cost
Request: {
  "cost_cny": 2500.00,
  "cost_idr": 5000000.00,
...
async def create_order_item(order_id: str, product_id: str, customer_id: str):
    ...
    # 4. KK 分配供应商后
    supplier_cost = await get_current_supplier_cost(product_id, supplier_id)
    order_item.selected_supplier_id = supplier_id
    order_item.supplier_cost_history_id = supplier_cost.id
    order_item.snapshot_cost_cny = supplier_cost.cost_cny
    order_item.estimated_profit_cny = price.price_cny - supplier_cost.cost_cny
```
...
SELECT 
    ...
    (SELECT IFNULL(SUM(snapshot_cost_cny), 0) 
     FROM order_items 
     WHERE order_id = o.id) AS supplier_cost,
...
**公式**:
```
订单项毛利 = 订单项收入 - 供应商成本 - 执行浮动成本（关联到此订单项的报销）
```
...
SELECT 
    ...
    oi.snapshot_cost_cny AS supplier_cost,
...
1. **识别需要迁移的产品**:
   ```sql
   SELECT id, vendor_id, price_cost_cny, price_cost_idr
   FROM products 
   WHERE vendor_id IS NOT NULL 
     AND (price_cost_cny IS NOT NULL OR price_cost_idr IS NOT NULL)
   ```
2. **创建供应商成本历史记录**:
   ```sql
   INSERT INTO supplier_cost_history (
   ...
   SELECT 
     ...
     1 AS version,
     COALESCE(price_cost_cny, 0) AS cost_cny,
     COALESCE(price_cost_idr, 0) AS cost_idr,
     created_at AS effective_start_at,
     1 AS is_current
   FROM products
   WHERE vendor_id IS NOT NULL 
     AND (price_cost_cny IS NOT NULL OR price_cost_idr IS NOT NULL);
...
```
GET /api/v1/products/{product_id}/suppliers/costs
Response: {
  "product_id": "...",
  "suppliers": [
    {
...
POST /api/v1/suppliers/{supplier_id}/products/{product_id}/costs
Request: {
  "cost_cny": 2500.00,
  "cost_idr": 5000000.00,
  "effective_from": "2025-01-01T00:00:00Z",
