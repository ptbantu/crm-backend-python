...
**设计**: Product × Customer Level 矩阵，存储对外销售价格（不是成本价格）
...
- `price_cny`: 人民币销售价格（对外售价）
- `price_idr`: 印尼盾销售价格（对外售价）
- `is_active`: 是否启用
- `effective_from`: 生效开始时间
- `effective_to`: 生效结束时间

**业务规则**:
- 同一产品在同一客户等级下只能有一个有效销售价格
...
Response: {
  "product_id": "...",
  "product_name": "公司注册",
  "sales_prices": [  // 销售价格列表（对外售价）
    {
      "level_code": "2",
      "level_name": "央企总部和龙头企业",
      "sales_price_cny": 50000.00,  // 销售价格（对外售价）
      "sales_price_idr": 100000000.00,
      "is_active": true,
      "effective_from": "2024-01-01T00:00:00Z",
      "effective_to": null
    },
...
**价格体系总结**:
- **销售价格** (`product_price_list`): 对外销售价格，根据客户等级分级，通过 `is_active` 和 `effective_from/effective_to` 控制生效时间
...
**文档版本**: v2.1  
**最后更新**: 2024-12-13  
**更新内容**: 添加内部交付支持，支持内部团队和外部供应商统一管理  
**维护人**: 开发团队
